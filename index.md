...
I then installed Arduino and the lessons that the OSOYOO recommendec to install and I eneded up installing one lesson in the arduino baord that was on the robot.

Some challenges that I faced were reading the schematic and following the video tutorial. It showed different steps of the schematics and also the video wasn't really descriptive about which piece go to what also same with the sckematic pictures. Another problem that I encounteted was trying to put on the nuts on the screw but sometimes the nuts wouldn't want to screw tightly.  I thnik the wire management is not necessary to fix but I want the robot to look clean. 

There is still some other parts to install but I mangened to install the base componets of the robot that were nesscary for it to run. I will contuine to follow the other lessons and install the parts that those lesson say to do.
